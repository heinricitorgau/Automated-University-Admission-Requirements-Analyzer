...
* **extractor.py**: 從 HTML/JSON 中提取結構化資料。
...
- 在主表與次順位表格加入 Country 欄位  
- 同步更新 Console / CSV / Excel / JSON  
- 建立 mock 測試腳本驗證版面  
...
- 第二順位預設關閉  
- 第一順位完成後詢問是否執行 Overall  
- 改為 conditional execution  
- 減少 API 呼叫與執行時間  
...
| 模式 | 穩定度 | 備註 |
| :--- | :--- | :--- |
| World Ranking | ⭐ 高 | API 穩定 |
| Subject Ranking | 🌗 中 | 偶發 Timeout |
| National Ranking | 🌗 中 | 大國資料量大 |
| Async 模式 | ⚠️ 中低 | 需控制並發 |
